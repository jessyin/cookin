...
import { Ingredient, Recipe, Tag } from "../models";
import { DirectionsListCreator, ImagePickerButton, IngredientsListCreator, TagInput, XIcon } from ".";
...
export default function RecipeForm({ recipe, onClose }: { recipe ?: Recipe, onClose: () => void }) {
  ...
  const [selectedTags, setSelectedTags] = useState<Set<Tag>>(recipe?.tags || new Set());
  ...
  const updateRecipeContexts = () => {
    ...
    selectedTags.forEach((tag) => newTags.add(tag));
    if (newTags.size != tags.size) setTags(newTags);

    const newPantry = new Set<string>([...pantry]);
    ingredients.forEach((ingredient) => { if (ingredient.name) newPantry.add(ingredient.name) });
    if (newPantry.size != pantry.size) setPantry(newPantry)
  
    if (newTags.size != tags.size || newPantry.size != pantry.size) editRecipeContext({ tags: newTags, pantry: newPantry, setTags, setPantry });
  }
  ...
  const recipeValid = name.length != 0 && directions.length != 0 && ingredients.length != 0 && cooktime > 0;
  ...
  return (
    <div className="z-10 fixed inset-0 bg-black/40 flex min-h-full items-center justify-center">
      ...
      <div className="relative flex flex-col w-1/2 z-20 max-h-3/4 rounded-lg bg-white shadow-xl">
        <button onClick={onClose}><XIcon styles="size-4 text-dark-gray absolute top-3 right-3 hover:text-red-1 hover:cursor-pointer active:text-red-dark" /></button>
        ...
        <div className="p-4 rounded-b-lg shadow-inner bg-light-gray/20">
          <button className="full-btn-blue w-full" onClick={recipe && onEditRecipe || onCreateRecipe} disabled={!recipeValid}>
